        ...
        if len(bbox_list) == 5:
            try:
                bbox_data = {'min_y': float(bbox_list[0]), 'min_x': float(bbox_list[1]), 'max_y': float(bbox_list[2]),
                             'max_x': float(bbox_list[3]), '3857': str(bbox_list[4])}
            except:
                raise Exception("Bbox coordinates must be numeric and contain comma separated EPSG:3857 after last coordinate")
            ...
            if bbox_data['3857'] != "EPSG:3857":
                raise Exception("Bbox must contain comma separated EPSG:3857 after last coordinate")
            ...
            raise Exception(
                "Projection must be EPSG:4326 (exactly 4 coordinates) or EPSG:3857 (exactly 4 coordinates plus EPSG:3857)")
    else:
        bbox_list = [i for i in bbox.split(',')]
        if len(bbox_list) == 4:
            try:
            ...
            raise Exception(
                "Projection must be EPSG:4326 (exactly 4 coordinates) or EPSG:3857 (exactly 4 coordinates plus 3857)")
...
def aoi_coverage(bbox, response):
    ...
    if "EPSG:3857" in bbox:
        bbox_list = [i for i in bbox.split(',')]
        bbox = ",".join([bbox_list[1], bbox_list[0], bbox_list[3], bbox_list[2]])
