...
def check_for_config():
    ...
    if os.path.exists(os.path.join(home_dir, '.MPS-config')):
        cli_interface = Interface()
    else:
        raise Exception("MPS config file not found. Please ensure that your config file is located in your home directory")
...
def config_file(username=None, password=None, domain=None):
    ...
    home_dir = os.path.expanduser("~")
    if os.path.exists(os.path.join(home_dir, '.MPS-config')):
        check = input("An MPS-config file already exists. Overwrite?[y/n]: ")
        if check.lower() == 'n':
    ...
    return click.echo("MPS-config file created. File is located in {}".format(home_dir))
...
def reset_password(password):
    ...
    if os.path.exists(os.path.join(home_dir, '.MPS-config')):
        ...
        return click.echo("No .MPS-config file found. Please create file")
    password = click.prompt('Enter new password', hide_input=True, confirmation_prompt=True)
    with open(os.path.join(home_dir, ".MPS-config"), 'r') as f:
        text = f.readlines()
